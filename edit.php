...
if($_SERVER["REQUEST_METHOD"] == "POST"){   
  if(empty($_POST["name"]) || empty($_POST["phone_number"])){
    ...
    $statement = $conn -> prepare("UPDATE contacts SET name = :name, phone_number = :phone_number, address = :address WHERE id = :id");
    $statement->execute([
      ":id" => $id,
      ":name" => $name,
        ...
        <div class="card-body">
          ...
          <form method="POST" action="edit.php?id=<?= $contact["id"] ?>">
            ...
            <div class="mb-3 row">
              ...
              <div class="col-md-6">
                <input value = "<?= $contact["address"] ?>" id="phone_number" type="tel" class="form-control" name="address" required autocomplete="address" autofocus>
              </div>
            </div>
